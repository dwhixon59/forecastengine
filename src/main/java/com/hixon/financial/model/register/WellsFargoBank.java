...
public class WellsFargoBank extends Bank {
   ...
   public Transaction loadFromCSV(CSVRecord record) throws ParseException, RegisterException, SQLException {
      ...
      int i = 2;
      for (; i < payeeTokens.length &&
              !payeeTokens[i].matches("^(0?[1-9]|1[0-2]){1}\\/(0?[1-9]|1[0-9]|2[0-9]|3[0-1]){1}.*"); i++)
         ;
   ...
   public String parseMerchantPayee() throws RegisterException, SQLException, ParseException {
      ...
      switch (firstFewWords) {
         ...
         case "ATM TRANSFER AUTHORIZED":
            ...
            if (i == payeeTokens.length) {
               ...
               accountNumber = resolver.resolveUnmatchedAccount(transaction.getPayee());
            } else {
