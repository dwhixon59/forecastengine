...
import static com.hixon.financial.model.EntityInt.SaveMethod.*;
...
public class ForecastTransaction extends IndependentEntity {
   ...
   // The select query:
   public static final String selectQuery = "select bin_to_uuid(a.idForecastTransaction) as 'id', a.remainingAmount, " +
           "a.plannedDate, a.firstOccurrence, bin_to_uuid(a.ForecastItem_idForecastItem) as 'idForecastItem' from " +
           "forecastdatabase.Forecast_Transaction a ";
   ...
   public ForecastTransaction(ResultSet rs) throws SQLException {
      super(false);
      this.id = UUID.fromString(rs.getString("idForecastTransaction"));
      this.remainingAmount = rs.getDouble("remainingAmount");
      this.plannedDate = Utility.SqlDateToCalendarDate(rs.getDate("plannedDate"));
      this.firstOccurrence = rs.getBoolean("firstOccurrence");
      this.idForecastItem = UUID.fromString(rs.getString("idForecastItem"));
   }
   ...
   private static ForecastTransactionIterator getForecastTransactionsForBudgetItem(UUID idBudgetItem, UUID idForecast)
      ...
      String selectQuery = "select bin_to_uuid(a.idForecastTransaction) as idForecastTransaction, a.remainingAmount, " +
              "a.plannedDate, firstOccurrence, bin_to_uuid(a.ForecastItem_idForecastItem) as 'idForecastItem', " +
              "b.category, b.payee, b.amount as 'plannedAmount' from ForecastDatabase.Forecast_Transaction a inner " +
              "join ForecastDatabase.Forecast_Item b on a.ForecastItem_idForecastItem = b.idForecastItem inner join " +
              "ForecastDatabase.Budget_Item c on b.BudgetItem_idBudgetItem = c.idBudgetItem where " +
              "a.remainingAmount <> 0 and c.idBudgetItem = uuid_to_bin('" + idBudgetItem + "') and b.Forecast_idForecast" +
              " = uuid_to_bin('" + idForecast + "') order by a.plannedDate asc ";
   ...
   private static ForecastTransactionIterator getForecastTransactionsForForecastItem(UUID idForecastItem)
           throws EntityException {
      String selectQuery = "select bin_to_uuid(a.idForecastTransaction) as idForecastTransaction, a.remainingAmount, " +
              "plannedDate, firstOccurrence, bin_to_uuid(a.ForecastItem_idForecastItem) as 'idForecastItem', " +
              "from ForecastDatabase.Forecast_Transaction where a.remainingAmount <> 0 and ForecastItem_idForecastItem =" +
              " uuid_to_bin('" + idForecastItem + "')  order by a.plannedDate asc ";
...
//   String updateQuery = "update ForecastDatabase.Forecast_Transaction set planned date = " +
//           Utility.calendarDateToSqlDateString(newDate) + "where ForecastItem_idForecastItem = uuid_to_bin('" +
//           idForecastItem + "')";
   ...
   public static void reconcile(Forecast forecast, Transaction transaction, List<TransactionSplit> splits,
                                TransactionResolver resolver) throws Exception, RegisterException, EntityException, BudgetException {
   ...
   public static ForecastTransaction getMatchingForecastTransaction(Forecast forecast, Transaction transaction,
           TransactionResolver resolver, TransactionSplit split) throws EntityException, Exception, BudgetException,
           RegisterException {
   ...
   public static ForecastTransaction deductSplitAmount(Forecast forecast, Transaction transaction,
                                                       TransactionResolver resolver, TransactionSplit split, ForecastTransaction forecastTransaction)
