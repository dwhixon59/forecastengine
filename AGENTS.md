...
Personal finance forecasting app (ForecastEngine). Java 21 + Maven + MySQL, strict MVC.
Tracks transactions, budgets, and generates cash-flow forecasts. See `README.md` and
`.github/copilot-instructions.md` for product context.

## Architecture: MVC with a swappable View layer

The single hard rule: **controllers NEVER interact with the user directly.** All I/O goes
through the `ViewInt` interface (`view/base/ViewInt.java`, ~990 lines). Controllers call
`view.say(...)`, `view.getYesOrNo(...)`, `view.selectByNameFromList(...)`, etc. This keeps
one controller codebase working across multiple front-ends: `view/cmdLine` (active),
`view/excel`, `view/csv`, `view/text`, `view/spreadsheetXml`.

- **Entry point:** `Main.java` loads `db.properties`, builds a `DatabaseConnectionManager`,
  then `new MainController(user, mgr, new ViewCmdline(), new fileBasedNotificationService())`.
- **Goals dispatch:** `MainController.run(String[] goals)` is a big `switch` over goal strings
  passed as program args, e.g. `dailyUpdate`, `manageData`, `importRegisterTransactions`,
  `createForecast`, `renderLongTermForecast`. Add a new feature = add a `case` here.
...
`Utility.setConnectionManager(...)`, `Utility.setUser(...)`, `Utility.setView(...)`.
Model/utility code reaches the current view through `Utility.getView()`. `Utility` is also the
home for all date/currency formatting (`calendarDateToStringDate`, `formatDollarAmount`,
`isEqualCurrency` — compare currency with the `CURRENCY_COMPARISON_THRESHOLD`, never `==`).
...
- Build: `mvn clean install` (Java 21, Lombok annotation processing configured in `pom.xml`).
- Test: `mvn test` (JUnit 5 + Mockito; surefire runs with `--enable-preview`). Tests in
  `src/test/java`; some legacy tests have `main()` methods and hit a real DB.
- Config: copy `src/main/resources/db.properties.example` → `db.properties` (git-ignored, never
  commit). Logging via `log4j2.properties`.
- Run a goal: launch `Main` with goal name(s) as program arguments (e.g. `dailyUpdate`).

## Conventions

- Package root: `com.hixon.financialApp`. Custom checked exceptions per module
  (`EntityException`, `RegisterException`, `BudgetException`, `ForecastException`).
